		...
		function() {
			...
			for ( var i in CKEDITOR.instances) {
				if (CKEDITOR.instances[i].checkDirty()) {
					// saving content
					save(CKEDITOR.instances[i].element.getId(),
...
function modal(header, body, modalId, modalWidth, modalHeight) {
	...
	jQuery(
			"<div id='"
					+ modalId
					+ "' class='modal hide fade'><div class='modal-header'> <a href='#' onclick='destroyModal("
					+ '"' + modalId + '"' + ")' class='close'>&times;</a>"
					+ header
					+ "</div><div id='modal-body-content' class='modal-body'>"
						...
						for ( var i in msg) {
							jQuery(
									"<option value='" + msg[i].id + "'>"
											+ msg[i].type + "</option>")
									.appendTo("#select-type-box");
						}
					}
				});
		jQuery(
				"<div class='form-actions'><a class='btn btn-primary' id='btn-valid-form' >Continue</a></div>")
				.appendTo("#modal-form");
		jQuery("#select-type-window").modal('show');
		jQuery('#btn-valid-form').click(
				function() {
...
function getDomainName() {
	return window.location.href.substr(7).substr(0,
			window.location.href.substr(7).indexOf("/"));
}
