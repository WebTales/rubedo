...
function swithToEditMode() {
	...
	jQuery('.radiogroup-edit').each(function() {
	 ...
	 jQuery(".complete-edition-btn").click(function(){
	 		...
	 		var modalUrl = "/backoffice/content-contributor?edit-mode=true&content-id="+targetContentId+"&workingLanguage="+jQuery("html").attr("lang");;
...
function addContent(type,typeId,queryId){
	...
	if(type=="manual")
		...
		jQuery('#btn-validate-ctselect').click(function() {
		    ...
		    jQuery('#btn-cancel-ctselect-form').unbind();
		    jQuery("#contentTypeSelectModal").modal("hide");
			var modalUrl = "/backoffice/content-contributor?typeId=" + selectedTypeId + "&queryId=" + queryId + "&current-page=" + jQuery('body').attr('data-current-page') + "&current-workspace=" + jQuery('body').attr('data-current-workspace')+"&workingLanguage="+jQuery("html").attr("lang");;
			var availWidth=window.innerWidth*(90/100);
			var properWidth=Math.min(1000,availWidth);
			var availHeight=window.innerHeight*(90/100);
			var properHeight=Math.max(400,availHeight);
		...
		jQuery("#contentTypeSelectModal").modal("show");
			...
			var modalUrl = "/backoffice/content-contributor?typeId=" + typeId + "&queryId=" + queryId + "&current-page=" + jQuery('body').attr('data-current-page') + "&current-workspace=" + jQuery('body').attr('data-current-workspace')+"&workingLanguage="+jQuery("html").attr("lang");
			var availWidth=window.innerWidth*(90/100);
			var properWidth=Math.min(1000,availWidth);
			var availHeight=window.innerHeight*(90/100);
			var properHeight=Math.max(400,availHeight);
