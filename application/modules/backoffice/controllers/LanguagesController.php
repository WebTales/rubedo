...
class Backoffice_LanguagesController extends Backoffice_DataAccessController
{
    ...
    public function getBoLanguagesAction(){
    	...
    	$result = Manager::getService('Languages')->getList($boLangFilter,array(array('property'=>$labelField,'direction'=>'ASC')));
    	$languagesArray = array();
    	foreach ($result['data'] as $languages){
    		$languagesArray[]=array('key'=>$languages['locale'],'label'=>$languages[$labelField]);
    	}
    	$this->_returnJson(array('data'=>$languagesArray,'success'=>true));
    }
